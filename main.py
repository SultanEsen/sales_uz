...
tab1, tab2, tab3, tab4 = st.tabs(["Загрузка данных", "Отчетность", "Базовые настройки", "Привязка аптек к рабочим местам"])
...
with tab3:
    ...
    with c1:
        ...
        with st.form("add_wp", clear_on_submit=True):
            wp_name = st.text_input("Номер (например, WP-101)")
...
# ==========================================
with tab4:
    st.subheader("Управление территориями по месяцам")

    existing_periods = [row[0] for row in conn.execute("SELECT DISTINCT period FROM monthly_pharmacy_type").fetchall()]
    ...
    # === ЛЕВАЯ ПАНЕЛЬ ===
    with col_left:
        # Жестко фиксируем высоту левой панели.
        # Если экран маленький, прокрутка будет внутри этого блока, а не на всей странице.
        with st.container(height=650, border=False):
            target_period = st.selectbox("Выберите период (ГГГГ-ММ):", all_target_periods, index=default_idx)

            st.markdown("**Скопировать привязки**")
            try:
            ...
            search_query = st.text_input("🔍 Поиск аптеки (введите ИНН или название и нажмите Enter):",
                                         help="Начните вводить текст для фильтрации списка")
            pharms_df = pd.read_sql("SELECT inn, IFNULL(pharmacy_name, 'Без названия') as name FROM pharmacies", conn)
            ...
            with st.form("edit_monthly_bind", clear_on_submit=True):
                sel_pharm = st.selectbox("Выберите аптеку:", pharm_options if pharm_options else ["Ничего не найдено"])
                p_type = st.selectbox("Тип аптеки:", [1, 2, 3, 4])
                workplaces = st.multiselect("Рабочие места:", avail_wps)
    ...
    with col_right:
        ...
        # Строго задаем такую же высоту, как и у левого блока (650px).
        # Теперь таблица будет прокручиваться ВНУТРИ себя, а левое меню будет стоять на месте.
...
with tab2:
    if conn.execute("SELECT COUNT(*) FROM raw_sales").fetchone()[0] == 0:
        ...
        if selected_periods:
            ...
            # ДОБАВЛЕНО: присоединяем таблицу pharmacies и берем pharmacy_name
            query = f"""
            SELECT 
            ...
            if not df_raw.empty:
                ...
                shares_dict = pd.read_sql("SELECT * FROM type3_shares", conn).set_index('unified_name').to_dict('index')
                dist_data = []
                ...
                for _, row in df_raw.iterrows():
                    ...
                    ph_name = row['pharm_name']  # ДОБАВЛЕНО: считываем название

                    wps = pharm_wps_dict.get((inn, period), [])


                    # ДОБАВЛЕНО: передаем ph_name в список
                ...
                # ДОБАВЛЕНО: колонка 'Название Аптеки'
                df_flat = pd.DataFrame(dist_data,
                                       columns=['Период', 'Город/Район', 'ИНН Аптеки', 'Название Аптеки', 'Препарат',
                                                'Линия', 'Рабочее место', 'Сумма'])
